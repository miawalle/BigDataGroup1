...
glm <- data.frame(R = 0.12*(1-exp(-50*glm_pred$X1))/(1-exp(-50)) +
                    0.24*(1-(1-exp(-50*glm_pred$X1))/(1-exp(-50))))

glm$b = (0.11852 - 0.05478*ln(glm_pred$X1))^2

glm$K = (LGD*pnorm(1-glm$R)^(-0.5)*qnorm(glm_pred$X1) + 
           (glm$R/(1-glm$R)^0.5*qnorm(0.999)) - glm_pred$X1*LGD)*
  (1-1.5*glm$b)*(1+(M-2.5)*glm$b)

glm$RWA = glm$K*12.5*EAD

#calculate RWA according to RWA formula and random forest

rf <- data.frame(R = 0.12*(1-exp(-50*rf_pred$X1))/(1-exp(-50)) +
                   0.24*(1-(1-exp(-50*rf_pred$X1))/(1-exp(-50))))

rf$b = (0.11852 - 0.05478*ln(rf_pred$X1))^2

rf$K = (LGD*pnorm(1-rf$R)^(-0.5)*qnorm(rf_pred$X1) + 
          (rf$R/(1-rf$R)^0.5*qnorm(0.999)) - rf_pred$X1*LGD)*
  (1-1.5*rf$b)*(1+(M-2.5)*rf$b)

rf$RWA = rf$K*12.5*EAD

#calculate RWA according to RWA formula and xgboost

xgb <- data.frame(R = 0.12*(1-exp(-50*xgb_pred$X1))/(1-exp(-50)) +
                   0.24*(1-(1-exp(-50*xgb_pred$X1))/(1-exp(-50))))

xgb$b = (0.11852 - 0.05478*ln(xgb_pred$X1))^2

xgb$K = (LGD*pnorm(1-xgb$R)^(-0.5)*qnorm(xgb_pred$X1) + 
          (xgb$R/(1-xgb$R)^0.5*qnorm(0.999)) - xgb_pred$X1*LGD)*
  (1-1.5*xgb$b)*(1+(M-2.5)*xgb$b)
...
stargazer(RWA_models, header = FALSE)
