...
```{r pre_boxplot, fig.asp=1, fig.width = 14, fig.cap="\\label{fig:pre_boxplot}Before cleaning"}
...
df_corp %>% 
  ...
  ggtitle("Boxplot of the distibution before cleaning") +
  theme(plot.title = element_text(hjust = 0.5))
...
df_cleaned %>% 
  ...
  geom_boxplot(show.legend = FALSE) +
  facet_wrap(~ metric, scales = "free") +
  ggtitle("Boxplot of the distibution after cleaning") +
  theme(plot.title = element_text(hjust = 0.5))
...
df_cleaned %>% 
  ...
  facet_wrap(~ metric, scales = "free") +
  ggtitle("Density plot of the distibution after cleaning") +
  theme(plot.title = element_text(hjust = 0.5))
...
$Correlation (R) = 0.12\cdot\frac{1-exp(-50 \cdot PD)}{1-exp(-50)}+0.24 \cdot [1-\frac{1-exp(-50 \cdot PD)}{1-exp(-50)}]-0.04 \cdot \frac{1-(s-5)}{45}$


$Maturity \space adjustment (b) = (0.11852-0.05478 \cdot ln(PF))^2$


$Capital \space requirement = [LGD \cdot N [(1-R)^{-0.5} \cdot G(PD) + (\frac{R}{1-R})^{0.5}\cdot G(0.999)]-PD \cdot LGD]\cdot (1-1.5 \cdot b)^{-1}\cdot(1+(M-2.5)\cdot b)$


$Risk - weighted\space asssets\space(RWA) = K \cdot 12.5 \cdot EAD$

```{r rwa_plots, fig.cap="\\label{fig:rwa_plots} RWA comparison"}
