...
```{r hist, cache=TRUE, fig.cap="\\label{fig:hist}Histogram of the distribution"}
...
```{r Boxplot_post_cleaning, fig.asp=1, fig.width = 14, fig.cap="\\label{fig:Boxplot_post_cleaning}Boxplot after cleaning"}
...
# For all models we will remove the equity, total_assets, revenue and amount_unpaid_dept because we want to work with relativ values.

df_reduced <- df_cleaned %>% 
  select(-equity, -total_assets, -revenue, -amount_unpaid_debt)

# Last thing we do before we split the data and start modelling is checking for mulitcolinarity.
# To do this we will be using vic function in car package.
...
mc_vif_2 # By checking again all looks okey
...
```{r var_imp_GLM, cache= TRUE, fig.cap="\\label{fig:var_imp_GLM}Variable importance"}
...
```{r ROC_glm, cache= TRUE, fig.cap="\\label{fig:ROC_glm}ROC plot for glm logistic"}
...
```{r var_imp_rf, cache = TRUE, fig.cap="\\label{fig:var_imp_rf}Variables importance Random Forest"}
...
```{r ROC_rf, cache = TRUE, fig.cap="\\label{fig:ROC_rf}ROC plot for Random Forest"}
...
```{r var_imp_xgb, Cache = TRUE, fig.cap="\\label{fig:var_imp_xgb}Variable importance Xgboost"}
...
```{r ROC_xgb, Cache = TRUE, fig.cap="\\label{fig:ROC_xgb}ROC plot for Xgboost"}
...
```{r xgb_tuning, echo=FALSE, out.width='100%', fig.cap="\\label{fig:xgb_tuning}Tuning of Xgboost"}
...
```{r model_comp, cache= TRUE, fig.cap="\\label{fig:model_comp}Comparing models with Accuracy and Kappa"}
...
```{r rwa_plots, cache= TRUE, fig.cap="\\label{fig:rwa_plots}RWA comparison"}
...
```{r impute_pmm, cache = TRUE, echo=FALSE, out.width='100%', fig.cap="\\label{fig:impute_pmm}Imputation using PMM"}
...
```{r impute_mean, echo=FALSE, cache = TRUE, out.width='100%', fig.cap="\\label{fig:impute_mean}Imputation using mean"}
