...
=======
### Data imbalance
When looking at our data, we notice that the variable \textit{default}, which will be the dependent variable in our prediction models, is imbalanced. That is, observations of defaulting companies are underrepresented, which will make it difficult to predict defaults. The underrepresentation of defaults is illustrated in figure \ref{fig:imbalance}. We will handle this imbalance when we implement the prediction models in section xx.
>>>>>>> 1f1d4fa1a9adc9af95a9d0b286d73ea174503738
...
```{r Density_post_cleaning, cache = TRUE, fig.asp=1, fig.width = 14, fig.cap="\\label{fig:Density_post_cleaning}Density plot after cleaning"}
...
```{r Boxplot_post_cleaning, cache = TRUE, fig.asp=1, fig.width = 14, fig.cap="\\label{fig:Boxplot_post_cleaning}Boxplot after cleaning"}
...
# we also remove gross based on AIC and significant value of the variable
...
saveRDS(model_rf, file = "glm.Rdata")
...
glm_pred$predict <- ifelse(glm_pred$Yes > 0.3, "Yes", "No")
...
```{r var_imp_xgb, fig.cap="\\label{fig:var_imp_xgb}Variable importance Xgboost"}
...
```{r ROC_xgb, fig.cap="\\label{fig:ROC_xgb}ROC plot for Xgboost"}
...
```{r model_comp, fig.cap="\\label{fig:model_comp}Comparing models with Accuracy and Kappa"}
